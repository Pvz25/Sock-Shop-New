...
# Step 1: Set Queue Policy using Management API
Write-Host "`n[Step 1] Configuring RabbitMQ queue policy via Management API..." -ForegroundColor Red
...
$policyJson = '{"pattern":"^shipping-task$","definition":{"max-length":3,"overflow":"reject-publish"},"apply-to":"queues"}'

$apiResult = kubectl -n sock-shop exec deployment/rabbitmq -c rabbitmq -- `
...
$policyCheck = kubectl -n sock-shop exec deployment/rabbitmq -c rabbitmq -- `
    curl -s -u guest:guest http://localhost:15672/api/policies 2>&1
...
Write-Host "`nChecking RabbitMQ queue depth via Management API..." -ForegroundColor Yellow
$queueInfo = kubectl -n sock-shop exec deployment/rabbitmq -c rabbitmq -- `
    curl -s -u guest:guest http://localhost:15672/api/queues/%2F/shipping-task 2>&1

if ($queueInfo -match '"messages":(\d+)') {
...
$ordersErrors = kubectl -n sock-shop logs deployment/orders --tail=50 2>&1 | Select-String "503|Service unavailable|shipping" | Select-Object -Last 10
...
Write-Host "✅ Queue policy removed via Management API" -ForegroundColor Green
...
$queueMasterLogs = kubectl -n sock-shop logs deployment/queue-master --tail=20 2>&1 | Select-String "Received shipment"
